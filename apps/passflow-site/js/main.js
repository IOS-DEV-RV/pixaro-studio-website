...
if (nav) {
  window.addEventListener('scroll', () => {
    if (window.scrollY > 40) {
      nav.style.background = 'rgba(14, 8, 24, 0.9)';
    } else {
      nav.style.background = 'rgba(26, 12, 32, 0.58)';
    }
  }, { passive: true });
}
...
if (contactForm) {
  contactForm.addEventListener('submit', (e) => {
    ...
    const subjectVal = document.getElementById('subject').value.trim() || 'Message from Passflow website';
...
document.querySelectorAll('.menu-link').forEach(link => {
  ...
  if (href === currentPath || (currentPath === '' && href === 'index.html')) {
    link.style.background = 'rgba(95,228,184,0.12)';
